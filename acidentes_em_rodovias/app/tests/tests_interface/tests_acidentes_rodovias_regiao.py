...
class AcidentesRodoviasRegiaoTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.browser.get(
            'http://127.0.0.1:' + self.porta + '/acidentes_rodovias/regiao')
        if (self.browser.title == u'Falha no carregamento da página'):
            sys.stderr.write("\nInicie a aplicação na porta " +
                             self.porta + " ou altere o atributo 'porta' no TestCase")
            exit(0)
        else:
    ...
    def test_estados_select(self):
        estados_list = [
            'Selecione um estado...', 'AC', 'AL', 'AP', 'AM', 'BA', 'CE', 'DF', 'ES', 'GO', 'MA', 'MT', 'MS',
    ...
    def test_municipios_acre(self):
        municipios_list = [
            u'Selecione um munic\xedpio...', 'ACRELANDIA', 'ASSIS BRASIL', 'BRASILEIA', 'BUJARI', 'CAPIXABA',
    ...
    def test_ocorrencias_acrelandia(self):
        ...
        resultados = self.browser.find_elements_by_xpath(
            "//div[@class='resultados']")

        for resultado in resultados:
            br_test = resultado.find_element_by_class(
                'br').get_attribute("text")
            self.assertEqual(br_test, br_excepected)

            municipio_test = resultado.find_element_by_class(
                'municipio').get_attribute("text")
            self.assertEqual(municipio_test, municipio_excepected)

            uf_test = resultado.find_element_by_class(
                'uf').get_attribute("text")
